<template>
  <article class="relative w-full h-full bg-lightbeige">
    <Container>
      <div class="relative w-full">
        <h1 ref="h1" class="overflow-hidden w-full flex justify-center pointer-events-none mb-12 mix-blend-difference text-white">
          ...
          <div class="relative w-full" :key="currentFilter">
            <div class="w-full grid md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-8 lg:gap-12 items-end">
              <template v-for="(item, key) in projects">
                <div
                  :data-ratio="item.cover && item.cover.ratio"
                  v-if="(item.cover && item.cover.image) || item.spacer"
                  :key="key"
                  :class="[
                    'relative w-full overflow-hidden',
                    item.spacer ? 'hidden lg:block pointer-events-none' : '',
                    item.cover && item.cover.ratio === ImageFormat.LANDSCAPE ? 'col-span-1 lg:col-span-2 h-full' : 'col-span-1 h-full'
                  ]"
                >
                  <div
                  ...
                  >
                    <div class="relative w-full h-full" @mouseenter.native="onLinkOver" @mouseleave.native="onLinkOut">
                      ...
                      <div v-if="!$mobile && !item.spacer" class="fullsize flex items-end px-4 pt-4 group cursor-pointer">
                ...
                <p v-if="$mobile && !item.spacer" class="font-medium mb-8 text-2xl">
